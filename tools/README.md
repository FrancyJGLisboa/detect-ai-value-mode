...
**Eval (`--eval`):** validates `evals.yaml` structure always; when `ANTHROPIC_API_KEY` is set, scores the description against `should_fire` / `should_not_fire` phrases and fails if recall or specificity drops below `--eval-threshold` (default 0.9). This is the discovery loss function — a clean skill that never triggers is still a failed skill.
...
- run: uv run skill-validator/validate_skill.py path/to/skill --eval
  env:
